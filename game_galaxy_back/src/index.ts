...
const initialData = {
  ...
  {
    "_id": {
      "$oid": "66130e833e64865272032999"
    },
    "user": "ljavier2022",
    "email": "ljavier2022@gmail.com",
    "password": "123456789",
    "updated_at": {
      "$date": "2024-04-07T21:22:11.104Z"
    },
...
function transformData(data: any) {
  return data.map((item: any) => {
    ...
    if (item.updated_at && item.updated_at.$date) {
      transformedItem.updated_at = new Date(item.updated_at.$date);
    }
    if (item.created_at && item.created_at.$date) {
      transformedItem.created_at = new Date(item.created_at.$date);
    }
  ...
  for (const collectionName of collections) {
    try {
      ...
      if (initialData[collectionName as keyof typeof initialData]) {
        const transformedData = transformData(initialData[collectionName as keyof typeof initialData]);
        for (const doc of transformedData) {
          const existingDoc = await collection.findOne({ _id: doc._id });
          if (!existingDoc) {
            await collection.insertOne(doc);
          } else {
            console.log(`El documento con _id ${doc._id} ya existe en la colección ${collectionName}`);
          }
        }
